...
def koliko(niz, datoteka):
    with open(datoteka, "w") as f:
        ...
        for ime in brez_ponovitev:
            print("{} {}".format(ime, kolikokrat_se_pojavi(niz,ime)), file = f)
# =====================================================================@001512=
# 3. podnaloga
...
def koliko_iz_datoteke(vhodna, izhodna):
    ...
    with open(vhodna) as f:
        for vrstica in f:
            imena = vrstica.strip().split(",")
            for ime in imena:
                vsa_imena.append(ime)
    brez_ponovitev = []
    for ime in vsa_imena:
        if ime not in brez_ponovitev:
            brez_ponovitev.append(ime)
    ...
    with open (izhodna, "w") as g:
        for ime in brez_ponovitev:
            print("{} {}".format(ime, vsa_imena.count(ime)), file = g)
# =====================================================================@001513=
# 4. podnaloga
...
def koliko_urejen(vhodna, izhodna):
    ...
    #dolocimo vsa imena brez ponovitev            
    brez_ponovitev = []
    for ime in vsa_imena:
        if ime not in brez_ponovitev: #imena se nismo videli
            brez_ponovitev.append(ime) #zato ga dodamo
    #naredimo seznam parov
    pari = []
    for ime in brez_ponovitev:
        pari.append((-vsa_imena.count(ime), ime))

    #uredimo ta seznam parov od najvecje pojavitve proti najmanjsi
    pari.sort()
    #zdaj pa ta imena zapisemo v izhodno datoteko
    with open (izhodna, "w") as g:
        for x, ime in pari:
            print("{} {}".format(ime, -x), file = g)
...
def _validate_current_file():
    ...
    try:
        ...
        token = 'Token 0388700367e3978b5b06033b517b2ca64bfad87e'
